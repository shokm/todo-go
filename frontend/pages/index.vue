  ...
  <div>
    ...
    <div v-for="n in range">
      <div v-if="posts[n - 1].status !== 1">
        id: {{ posts[n - 1].id }}, todo: {{ posts[n - 1].todo }}
      </div>
    </div>
    ...
    <h3>作成</h3>
    <div>
      <lavel>Status: <input v-model="inputStatus" type="text" /></lavel>
      <lavel>Todo: <input v-model="inputTodo" type="text" /></lavel>
      <input type="button" @click="postTodo()" />
    </div>
...
export default Vue.extend({
  ...
  data() {
    return {
      inputStatus: 0,
      inputTodo: 'test8_test'
    }
  },
  methods: {
    postTodo() {
      axios.post('/api/v1/todo/8', {
